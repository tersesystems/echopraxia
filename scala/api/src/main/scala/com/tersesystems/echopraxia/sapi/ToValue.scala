...
/**
 * The ToValue trait, used for turning scala things into Field.Value.
 * 
...
@FunctionalInterface
trait ToValue[-T] {
  def toValue(t: T): Field.Value[_]
}

object ToValue {
  def apply[T: ToValue]: ToValue[T] = implicitly[ToValue[T]]

  private def convert[T](f: T => Field.Value[_]): ToValue[T] = (value: T) => f.apply(value)

  implicit val valueToValue: ToValue[Field.Value[_]] = ToValue.convert(identity)

  implicit val stringToStringValue: ToValue[String] = ToValue.convert(Field.Value.string)

  implicit def numberToValue[N <: Number]: ToValue[N] = ToValue.convert(Field.Value.number(_))
  implicit val byteToValue: ToValue[Byte]             = ToValue.convert(Field.Value.number(_))
  implicit val shortToValue: ToValue[Short]           = ToValue.convert(Field.Value.number(_))
  implicit val intToValue: ToValue[Int]               = ToValue.convert(Field.Value.number(_))
  implicit val longToValue: ToValue[Long]             = ToValue.convert(Field.Value.number(_))
  implicit val doubleToValue: ToValue[Double]         = ToValue.convert(Field.Value.number(_))
  implicit val floatToValue: ToValue[Float]           = ToValue.convert(Field.Value.number(_))
  implicit val bigIntToValue: ToValue[BigInt]         = ToValue.convert(Field.Value.number(_))
  implicit val bigDecimalToValue: ToValue[BigDecimal] = ToValue.convert(Field.Value.number(_))

  implicit val booleanToBoolValue: ToValue[Boolean] = ToValue.convert(Field.Value.bool(_))
  implicit val javaBoolToBoolValue: ToValue[java.lang.Boolean] = ToValue.convert(Field.Value.bool)

  implicit def throwableToValue[T <: Throwable]: ToValue[T] = ToValue.convert(Field.Value.exception)

  implicit def optionValue[V: ToValue]: ToValue[Option[V]] = {
    case Some(v) =>
      implicitly[ToValue[V]].toValue(v)
    case None =>
      Field.Value.NullValue.instance
  }
...
trait ToArrayValue[-T] {
  def toArrayValue(t: T): Field.Value.ArrayValue
}
...
object ToArrayValue {
  ...
  implicit val identityArrayValue: ToArrayValue[Field.Value.ArrayValue] = identity(_)
...
trait ToObjectValue[-T] {
  def toObjectValue(t: T): Field.Value.ObjectValue
}
...
object ToObjectValue {
  ...
  //        case (k, v) =>
  //          Field.Value.keyValue(k, v.toString)
  //      }.toSeq
