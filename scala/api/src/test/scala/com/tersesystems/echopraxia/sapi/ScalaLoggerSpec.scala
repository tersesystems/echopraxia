...
class ScalaLoggerSpec extends AnyFunSpec with BeforeAndAfterEach with Matchers {
  ...
  describe("withFields") {
    it("should use withFields") {
      logger.withFields(fb => fb.onlyString("derp", "herp"))
    }
  }

  describe("tuple") {

    it("should log using a single tuple using onlyKeyValue") {
      logger.debug("single tuple {}", _.onlyKeyValue("foo" -> "bar"))
      matchThis("single tuple {}")
    }

    it("should log using a single tuple using onlyValue") {
      logger.debug("single tuple {}", _.onlyValue("foo" -> "bar"))
      matchThis("single tuple {}")
    }
  ...
  describe("seq") {
    ...
    it("should log using a Seq of String") {
      ...
      logger.debug("seq {}", _.onlyArray("someSeq", seq))
    ...
    it("should log using a Seq of boolean") {
      logger.debug(
        ...
        { fb =>
          import fb._
          onlyArray("someSeq", Seq(true, false, true))
        }
      )
  ...
  describe("more complex ToValue") {
    ...
    it("should log a Try") {
      logger.debug("try {}", fb => fb.only(fb.keyValue("result", Try(true))))
      matchThis("try {}")
    }

    it("should log a Success") {
      logger.debug("success {}", fb => fb.only(fb.keyValue("result", Success(true))))
      matchThis("success {}")
    }

    it("should log an option try") {
      logger.debug("option try {}", fb => fb.only(fb.keyValue("result", Option(Try(true)))))
      matchThis("option try {}")
    }

    it("should log an Either") {
      val either: Either[Int, String] = Either.cond(System.currentTimeMillis() > 1, "foo", 1)
      logger.debug("either {}", _.onlyKeyValue("result" -> either))
      matchThis("either {}")
    }
  ...
  describe("instant and person") {
    ...
    it("should log an instant as a string") {
      logger.debug(
        ...
        fb => {
          import fb._
          fb.only(keyValue("iso_timestamp" -> Instant.now()))
        }
      )
    ...
    it("should log a person as an object") {
      logger.debug(
        ...
        fb => {
          import fb._
          fb.onlyObj("owner", keyValue("person" -> Person("Eloise", 1)))
        }
      )
    ...
    it("should handle Option[Foo] as Null") {
      ...
      logger.debug(
        ...
        fb => {
          ...
          fb.only(fb.value("foo", optPerson))
        }
      )
  ...
  class MyFieldBuilder extends FieldBuilder {
    ...
    def onlyInstant(name: String, i: Instant): util.List[Field] =
      onlyKeyValue(name, instantValue(i))
